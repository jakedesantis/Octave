...
%fprintf('Size of X_grad is %f x %f\n', size(X_grad,1), size(X_grad,2));
%fprintf('It should be %f x %f\n', size(X,1), size(X,2));

%Theta_grad = ((X*Theta').*R - Y.*R)'*X;
Theta_grad = ((X*Theta').*R - Y.*R)'*X .+ lambda*Theta;

%fprintf('Size of Theta_grad is %f x %f\n', size(Theta_grad,1), size(Theta_grad,2));
